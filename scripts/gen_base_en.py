...
from src.utils.prompts import PREDICTION_BASE_PROMPT_TEMPLATE
...
def parse_output(raw_output: str) -> Tuple[str, str]:
    ...
    """
    try:
        # Extract prediction [A-D] inside <prediction> tags
        ...
        # Extract explanation inside <explanation> tags (can span multiple lines)
...
def main():
    ...
    for i, row in tqdm(df.iterrows(), total=len(df), desc="Processing", unit="row"):
        ...
        prompt = build_prompt(row, PREDICTION_BASE_PROMPT_TEMPLATE)
    ...
    out_df = out_df[cols]
    out_df.to_csv(cfg["output"], index=True)
    logging.info(f"Results saved to {cfg['output']}")
